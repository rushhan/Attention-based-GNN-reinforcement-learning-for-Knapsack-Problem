...
class AttentionModel(nn.Module):
    ...
    def __init__(self,
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            ...
            if self.is_pctsp:
                node_dim = 2  # x, y, expected_prize, penalty
            else:
    ...
    def forward(self, input, return_pi=False):
        ...
        cost, mask = state.get_final_cost()
    ...
    def _init_embed(self, input):
        ...
        if self.is_vrp or self.is_orienteering or self.is_pctsp:
            if self.is_vrp:
                ...
                features2 = ('null_value')

                #print (input['null_value'])
            elif self.is_orienteering:
                features = ('prize', )
            else:
                assert self.is_pctsp
                features = ('deterministic_prize', 'penalty')
            trm=torch.cat((input['null_weight'], input['null_value']                    
                    ), -1)
            #print(trm.shape)
            trm2 =self.init_embed_depot(trm)
            trm1 =torch.cat((
                        
                        input['weight'][:,:,None], input['value'][:,:,None]
                                            ), -1)
            #print ("the shape is ",trm1.shape)
            trm3 = self.init_embed(trm1)
            #print (trm2.shape,trm3.shape)
            return torch.cat(
                (trm2[:, None, :]
                    ,trm3
    ...
    def _inner(self, input, embeddings):
        ...
        #print ("the shapes are thse",mask_dum.shape,.shape)

        while not (torch.sum(~mask[:,:,1:]).float()==0.0).item(): 
            #print ("i is this one",i)
        # need to change this condition to mask ==all true except fro the null one

        

            log_p, mask = self._get_log_p(fixed, state)
            
            #print ("the mask is this one",mask)
            # Select the indices of the next nodes in the sequences, result (batch_size) long
            selected = self._select_node(log_p.exp()[:, 0, :], mask[:, 0, :])  # Squeeze out steps dimension
            #print ("the mask is this three",mask,selected,state)
            state = state.update(selected)
            #print ("the mask is this four",mask)
            ...
            #print ("the mask is this teo",mask)
